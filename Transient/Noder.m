function [nodeVector, equationMatrix] = Noder( thisEquationMatrix, A, VoltageOnly, nodeNumber, ithNode, branchBaad)
    syms s t
    ...
    while 1
        if isequaln(tempMat(i,:), branchBaad) == 1
            tempMat(i,:) = [];
            i = i - 1;
        end
        i = i + 1;
            ...
            for k = 1:size(tempMat, 1)
                if tempMat(k, 4) == sym(j) || tempMat(k, 5) == sym(j)
                    switch (double(tempMat(k, 1)))
                        ...
                        case 4
                            % for inductor
                            nodeVector(j-1) = nodeVector(j-1) - (1 / (s * tempMat(k,3)));

                        case 5
                            % for capacitor
                            nodeVector(j-1) = nodeVector(j-1) - (s * tempMat(k,3));   
                    end
                end
            end
        else
            ...
            for k = 1:size(tempMat, 1)
                switch (double(tempMat(k, 1)))
                    ...
                    case 4
                        ...
                        % for initial conditions of inductor
                        if tempMat(k, 4) == sym(j)
                            nodeVector(end) = nodeVector(end) - tempMat(k,6) / s;
                        else
                            nodeVector(end) = nodeVector(end) + tempMat(k,6) / s;
                        end
                    case 5
                        ...
                        % for initial conditions of capacitor
                        if tempMat(k, 4) == sym(j)
                            nodeVector(end) = nodeVector(end) + tempMat(k,6) * tempMat(k,3);
                        else
                            nodeVector(end) = nodeVector(end) - tempMat(k,6) * tempMat(k,3);
                        end
