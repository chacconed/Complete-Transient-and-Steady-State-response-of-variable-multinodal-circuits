...
%     pointsPer = 20; % points per interval of log axis for freq response
    ...
    switch (flag)
        ...
        case 0
            ...
            N = (k_end - k_start) * pointsPer; % number of points from start to end frequency

            freq = logspace(k_start, k_end, N);
            vNodal_vals = zeros(nodeNumber, length(freq));
            vBranch_vals = zeros(branchNumber, length(freq));
            iBranch_vals = zeros(branchNumber, length(freq));
            tic
            for j = 1:length(freq)
                w = 2*pi*freq(j);
